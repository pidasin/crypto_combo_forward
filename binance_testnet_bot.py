...
if open_p:
    ...
    for p in sorted(open_p,key=lambda x:-abs(float(x['unrealizedProfit']))):
        ...
        positions_snapshot.append(dict(symbol=p['symbol'],qty=amt,entry=ep,upnl=up,pct=pct))
...
filled=[]   # (sym, side, qty, fill_px, notional, orderId) — 逐筆成交紀錄
if not orders:
    print("\n  沒有需要調整的部位")
else:
    print(f"\n  共 {len(orders)} 筆單:")
    for sym,side,qty in orders:
        if DRY_RUN:
            ...
            try:
                ...
                fill_px=float(res.get('avgPrice') or 0) or float(public('/fapi/v1/ticker/price',{'symbol':sym})['price'])
                filled.append((sym,side,qty,fill_px,qty*fill_px,res.get('orderId')))
...
if not DRY_RUN:
    ...
    print(f"\n  ✅ 已記錄 (bot_log.csv)")
    # 【新增】逐筆訂單明細, append-only
    if filled:
        newf2=not os.path.exists(ORDERS_LOG)
        with open(ORDERS_LOG,'a',newline='',encoding='utf-8-sig') as f:
            wcsv=csv.writer(f)
            if newf2: wcsv.writerow(['日K','執行UTC','幣種','方向','數量','成交價','名目USDT','orderId'])
            now=pd.Timestamp.utcnow().strftime('%Y-%m-%d %H:%M')
            for sym,side,qty,px_,notional,oid in filled:
                wcsv.writerow([str(D.date()),now,sym,side,qty,round(px_,6),round(notional,2),oid])
        print(f"  ✅ 已記錄 {len(filled)} 筆逐筆交易 (orders_log.csv)")
else:
